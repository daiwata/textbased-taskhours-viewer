            ...
            for category, tasks in categories.items():
                for task, task_data in tasks.items():
                    for key, val in task_data.items():
                        try:
                            results[year_month]['month_total'][key] = results[year_month].get('month_total', {}).get(key, 0) + val
                            results[year_month][category]['category_total'][key] = results[year_month][category].get('category_total', {}).get(key, 0) + val
                            results[year_month][category][task][key] = results[year_month][category][task].get(key, 0) + val
                        except TypeError:
